...
def calc_chk(data): return PASSWORDS[sum(data) & 0xFF]
...
class GameEngine:
    def __init__(self, net, ui, diff, num_players, target_score):
        ...
        self.base_tiles += (num_players - 2) # Mai mulți jucători = pattern-uri ușor mai mari de la început
    ...
    def _run(self):
        ...
        while True:
            ...
            self.state = "ACTION"; self.ui.update_status("🟢 OCHI ÎNCHIS! ACȚIONAȚI!")
            st = time.time()
            while time.time() - st < 7.0:
                ...
                if len(self.guessed) >= len(self.pattern):
                    ...
                    break
                time.sleep(0.1)
            else: self.ui.update_status("⌛ Timp expirat!")
            
            if self.state == "WIN":
                self.ui.update_status("🏆 MISIUNE ÎNDEPLINITĂ! 🏆")
                self.audio.play_correct() 
                # Loop infinit de victorie verde
                while True:
    ...
    def on_press(self, ch, l):
        if l == 0 or self.state == "WIN": return
...
class EsportsApp(ctk.CTk if CTK_AVAILABLE else tk.Tk):
    def __init__(self):
        ...
        # Bind Ieșire și Simulator Toggle
        self.bind("<Escape>", lambda e: self.destroy())
        self.bind("<F12>", self.toggle_simulator)

        # Rețea: Autodetectare Ethernet (Family Friendly)
        ...
        self.target_score = 30 # Default

        # Canvas Fundal Animat (Matrix Rain)
    ...
    def auto_detect_ip(self):
        """Caută automat placa de rețea care este conectată la hardware"""
        try:
            import psutil
    ...
    def toggle_simulator(self, event=None):
        """Hidden feature pentru debug (F12)"""
    ...
    def build_frames(self):
        ...
        self.add_label(self.players_frame, "CÂȚI JUCĂTORI PARTICIPĂ?", self.f_info, "white").pack(pady=(0, 60))
        ...
        self.add_button(self.players_frame, "2 JUCĂTORI", "#00BFFF", "#00E5FF", lambda: self.select_players(2), **btn_kw).pack(pady=15)
        self.add_button(self.players_frame, "3 JUCĂTORI", "#FF0055", "#FF3377", lambda: self.select_players(3), **btn_kw).pack(pady=15)
        self.add_button(self.players_frame, "4 JUCĂTORI", "#00E676", "#69FF9E", lambda: self.select_players(4), **btn_kw).pack(pady=15)
        ...
        self.add_label(self.diff_frame, "NIVEL DE DIFICULTATE", self.f_super, "white").pack(pady=(0, 60))

        self.add_button(self.diff_frame, "UȘOR", "transparent", "#00E676", lambda: self.show_details("EASY", 30), text_color="#00E676", **btn_kw).pack(pady=20)
        self.add_button(self.diff_frame, "MEDIU", "transparent", "#00BFFF", lambda: self.show_details("MEDIUM", 50), text_color="#00BFFF", **btn_kw).pack(pady=20)
        self.add_button(self.diff_frame, "GREU", "transparent", "#FF0055", lambda: self.show_details("HARD", 75), text_color="#FF0055", **btn_kw).pack(pady=20)
        ...
        header.pack(fill="x", padx=40, pady=40, ipadx=20, ipady=20)
        
        self.lbl_hud_players = self.add_label(header, "JUCĂTORI: 4", self.f_btn, "#00E5FF")
        self.lbl_hud_players.pack(side="left")
        self.lbl_hud_diff = self.add_label(header, "MOD: HARD", self.f_btn, "#FF003C")
        self.lbl_hud_diff.pack(side="right")
        
        # Scor uriaș pe mijloc
        ...
        self.add_label(center_box, "SCOR CURENT", self.f_title, "#888").pack(pady=(50, 0))
        ...
        self.lbl_hud_target.pack(pady=(0, 50))
        
        # Status Box (în loc de logs)
        ...
        status_box.pack(fill="x", padx=40, pady=40, ipady=30)
    ...
    def hide_all(self):
        for f in [self.players_frame, self.diff_frame, self.details_frame, self.load_frame, self.score_frame]:
            f.place_forget()
    ...
    def prep_game(self):
        ...
        if CTK_AVAILABLE:
            ...
            self.start_engine() # Fallback rapid daca nu exista ctk
