...
class AudioManager:
    def __init__(self):
        ...
        try:
            ...
            det_p = os.path.join(sounds_dir, "WrongButton.mp3")
            cor_p = os.path.join(sounds_dir, "Correct.mp3")
            self.snd_detected = pygame.mixer.Sound(det_p) if os.path.exists(det_p) else None
            self.snd_correct = pygame.mixer.Sound(cor_p) if os.path.exists(cor_p) else None
        except:
            self.enabled = False
...
class NetManager:
    def __init__(self, target_ip, local_ip):
        ...
        if self.local_ip not in ["127.0.0.1", "0.0.0.0"]:
    ...
    def push(self, led_dict):
        ...
        for (ch, logic_led), (r, g, b) in led_dict.items():
            ...
            if is_simulator: 
                hw_led, c1, c2, c3 = logic_led, g, r, b
            else: 
                hw_led, c1, c2, c3 = (10 if logic_led == 0 else logic_led - 1), r, g, b
            idx = hw_led * 12 + (ch - 1)
...
class GameEngine:
    ...
    def _draw(self):
        f = {}
        for ch in range(1, 5):
            # 1. Panourile de joc (1-10)
            for l in range(1, 11):
                col = (0, 0, 0)
                if self.state in ["ROULETTE", "WARNING"]:
                    col = (255, 0, 0)
                elif self.state == "GAZE":
                    if (ch, l) in self.pattern:
                        col = (0, 255, 255)
                    elif (ch, l) in self.frenzy:
                        col = (200, 0, 255)
                    else:
                        col = (255, 255, 255)
                elif self.state == "ACTION":
                    if (ch, l) in self.guessed:
                        col = (0, 255, 0)
                    elif (ch, l) in self.wrong_guessed:
                        col = (255, 0, 0)
                    else:
                        col = (255, 255, 255)
                f[(ch, l)] = col

            # 2. Logica pentru OCHI (index 0)
            ec = (0, 0, 0) 
            
            if self.state == "ROULETTE": 
                # Ochiul alb care se plimbă
                ec = (255, 255, 255) if ch == self.eye_wall else (0, 0, 0)
            
            elif self.state == "WARNING": 
                # Ochiul galben care pâlpâie unde s-a oprit ruleta
                ec = (255, 255, 0) if (ch == self.eye_wall and int(time.time()*10)%2==0) else (0, 0, 0)
            
            elif self.state == "GAZE": 
                # DOAR ochiul selectat devine roșu (cel care te „vede”)
                ec = (255, 0, 0) if ch == self.eye_wall else (0, 0, 0)
            
            elif self.state == "ACTION":
                # Toți ochii sunt stinși în timpul mișcării (cerința ta anterioară)
                ec = (0, 0, 0)
    ...
    def _run(self):
        ...
        # --- PORNEȘTE MUZICA CHIAR LA ÎNCEPUT ---
        self.audio.play_bgm() 

        st = time.time()
        while time.time() - st < 3.0 and self.running:
            frame = {(c, l): (0, 255, 255) for c in range(1, 5) for l in range(1, 11)}
            for c in range(1, 5):
                frame[(c, 0)] = (0, 255, 255) if int(time.time()*5)%2==0 else (0, 0, 0)
            self.net.push(frame)
            time.sleep(0.1)
        ...
        while self.running:
            ...
            while time.time() - st < self.t_warn and self.running:
                self._draw()
                time.sleep(0.1)

            # 3. GAZE
            self.state = "GAZE"
            self.ui.update_status("🔴 OCHI DESCHIS!")
            st = time.time()
            ...
            if self.state == "STUN":
                for _ in range(8):
                    ...
                    time.sleep(0.2); self.net.push({(c,l): (0,0,0) for c in range(1,5) for l in range(11)})
                    time.sleep(0.2)
                continue

            # 4. ACTION
            self.state = "ACTION"
            self.ui.update_status("🟢 OCHI ÎNCHIS!")
            st = time.time()
            round_success = False
            ...
            if round_success:
                ...
                self.ui.update_status("🎉 CORECT! REPAUS 5 SEC...")
                
                # EFECT VERDE + SUNET CORECT
                self.audio.play_correct()
                self.net.push({(c, l): (0, 255, 0) for c in range(1, 5) for l in range(11)})
                
                time.sleep(5.0) # Aici stă 5 secunde pe verde
                
                if self.score >= self.target_score:
                    self.state = "WIN"
                    break 
            else:
                self.ui.update_status("⌛ TIMP EXPIRAT!")
                time.sleep(1.0)

        # 5. FINAL (WIN) - FĂRĂ stop_bgm()
        if self.state == "WIN" and self.running:
            self.ui.update_status("🏆 MISIUNE FINALIZATĂ! 🏆")
            # Muzica CONTINUĂ să cânte aici!
            while self.running:
                self.net.push({(c, l): (255, 0, 0) for c in range(1, 5) for l in range(11)})
                time.sleep(0.5)
    ...
    def on_press(self, ch, l):
        if not self.running or self.state in ["OFF", "WIN", "ROULETTE", "WARNING", "STUN", "GAMEOVER"]: return
        
        if self.state == "GAZE":
            if l == 0 and ch == self.eye_wall:
                self.score = max(0, self.score - 5)
                self.ui.update_score(self.score)
                self.state = "STUN"
                self.ui.update_status("☠️ TE-A VĂZUT OCHIUL! (-5 Puncte)")
                self.audio.play_detected()
            elif l > 0 and self.frenzy and (ch, l) in self.frenzy:
                self.score += 3
                self.frenzy.remove((ch, l))
                self.ui.update_score(self.score)
            elif l > 0:
                self.score = max(0, self.score - 5)
                self.ui.update_score(self.score)
                self.state = "STUN"
                self.ui.update_status("☠️ MIȘCARE DETECTATĂ! (-5 Puncte)")
                self.audio.play_detected()
            ...
            if (ch, l) in self.pattern:
                if (ch, l) not in self.guessed: 
                    self.guessed.add((ch, l))
                    self.score += 1
                    self.ui.update_score(self.score)
            else:
                if (ch, l) not in self.wrong_guessed:
                    self.wrong_guessed.add((ch, l))
                    self.score = max(0, self.score - 2)
                    self.ui.update_score(self.score)

class ScoreboardWindow(ctk.CTkToplevel if CTK_AVAILABLE else tk.Toplevel):
    def __init__(self, master, players, diff, target):
        super().__init__(master)
        self.title("Evil Says - Monitor Jucători")
        self.geometry("1024x768")
        if CTK_AVAILABLE: self.configure(fg_color="#0A0A12")
        else: self.configure(bg="#0A0A12")
        ...
        f_hud_score = ("Courier New", 180, "bold")
        f_hud_status = ("Consolas", 35, "bold")

        header = ctk.CTkFrame(self, fg_color="#12121A", corner_radius=20, border_width=2, border_color="#333") if CTK_AVAILABLE else tk.Frame(self, bg="#12121A")
        header.pack(side="top", fill="x", padx=40, pady=40, ipadx=20, ipady=20)
        self.lbl_hud_players = ctk.CTkLabel(header, text=f"JUCĂTORI: {players}", font=f_btn_small, text_color="#00E5FF") if CTK_AVAILABLE else tk.Label(header, text=f"JUCĂTORI: {players}", font=f_btn_small, fg="#00E5FF", bg="#12121A")
        self.lbl_hud_players.pack(side="left")
        self.lbl_hud_diff = ctk.CTkLabel(header, text=f"MOD: {diff}", font=f_btn_small, text_color="#FF003C") if CTK_AVAILABLE else tk.Label(header, text=f"MOD: {diff}", font=f_btn_small, fg="#FF003C", bg="#12121A")
        self.lbl_hud_diff.pack(side="right")

        status_box = ctk.CTkFrame(self, fg_color="#12121A", corner_radius=20, border_width=2, border_color="#00E5FF") if CTK_AVAILABLE else tk.Frame(self, bg="#12121A", bd=3)
        status_box.pack(side="bottom", fill="x", padx=40, pady=(0, 40), ipady=30)
        self.lbl_hud_status = ctk.CTkLabel(status_box, text="AȘTEPTARE...", font=f_hud_status, text_color="#00E5FF") if CTK_AVAILABLE else tk.Label(status_box, text="AȘTEPTARE...", font=f_hud_status, fg="#00E5FF", bg="#12121A")
        self.lbl_hud_status.pack()

        center_box = ctk.CTkFrame(self, fg_color="transparent") if CTK_AVAILABLE else tk.Frame(self, bg="#0A0A12")
        center_box.pack(expand=True, fill="both")
        lbl_sc = ctk.CTkLabel(center_box, text="SCOR CURENT", font=f_title, text_color="#888") if CTK_AVAILABLE else tk.Label(center_box, text="SCOR CURENT", font=f_title, fg="#888", bg="#0A0A12")
        lbl_sc.pack(pady=(20, 0))
        self.lbl_hud_score = ctk.CTkLabel(center_box, text="0", font=f_hud_score, text_color="white") if CTK_AVAILABLE else tk.Label(center_box, text="0", font=f_hud_score, fg="white", bg="#0A0A12")
        self.lbl_hud_score.pack()
        self.lbl_hud_target = ctk.CTkLabel(center_box, text=f"TARGET: {target}", font=f_title, text_color="#ffd000") if CTK_AVAILABLE else tk.Label(center_box, text=f"TARGET: {target}", font=f_title, fg="#ffd000", bg="#0A0A12")
...
class EsportsApp(ctk.CTk if CTK_AVAILABLE else tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Evil Says - Game Master")
    ...
    def toggle_simulator(self, event=None):
        ...
        if self.is_simulator:
            ...
            if CTK_AVAILABLE: self.lbl_sim_status.configure(text="MOD: SIMULATOR ACTIVAT", text_color="#00E5FF")
            else: self.lbl_sim_status.config(text="MOD: SIMULATOR ACTIVAT", fg="#00E5FF")
        else:
            self.target_ip = HARDWARE_TARGET_IP
            self.local_ip = self.auto_detect_ip()
            if CTK_AVAILABLE: self.lbl_sim_status.configure(text="MOD: HARDWARE (Apasă F12 pentru Simulator)", text_color="#555555")
            else: self.lbl_sim_status.config(text="MOD: HARDWARE (Apasă F12 pentru Simulator)", fg="#555555")
    ...
    def build_frames(self):
        ...
        self.add_button(self.players_frame, "🧪 TEST CULORI RGB", "#1A1A24", "#ffd000", self.start_rgb_test, width=350, height=60, corner_radius=20, border_width=2, font=self.f_btn_small, text_color="#ffd000", hover_color="#2A2A35").pack(pady=10)
        ...
        self.lbl_sim_status = self.add_label(self.players_frame, "MOD: HARDWARE (Apasă F12 pentru Simulator)", ("Helvetica", 14, "bold"), "#555555")
        self.lbl_sim_status.pack(pady=(20, 0))
        ...
        info_text = ("1. ROULETTE & MEMORARE (CYAN)\nOchii se învârt. Când se opresc, panourile vor lumina în CYAN.\nNU VĂ MIȘCAȚI ȘI NU ATINGEȚI PEREȚII!\n\n"
                     "2. ACȚIUNE (VERDE)\nTiparul CYAN se va stinge.\nApăsați locațiile reținute.\n\n"
                     "3. FEBRA DE AUR (MOV)\nLuminile MOV sunt bonus (+3 Pcte).\n\n"
                     "Orice mișcare la Ochi declanșează alarma!")
        ...
        self.test_frame = ctk.CTkFrame(self, fg_color="#12121A", corner_radius=40, border_width=3, border_color="#ffd000") if CTK_AVAILABLE else tk.Frame(self, bg="#12121A", bd=5)
        if CTK_AVAILABLE:
            self.test_frame.configure(width=1000, height=600)
            self.test_frame.pack_propagate(False)
        self.add_label(self.test_frame, "TEST DE CALIBRARE CULORI", self.f_title, "#ffd000").pack(pady=(50, 20))
        self.add_label(self.test_frame, "Modificați funcția `push` din NetManager dacă culorile nu corespund.", self.f_desc, "white").pack()
        self.lbl_test_col = self.add_label(self.test_frame, "SE TRIMITE: ROȘU", self.f_super, "#FF0000")
        self.lbl_test_col.pack(expand=True)
        self.add_button(self.test_frame, "OPREȘTE TESTUL", "#FF003C", "#FF003C", self.stop_rgb_test, width=300, height=80, corner_radius=20, font=self.f_btn, text_color="white").pack(pady=(0, 50))
        ...
        self.gm_frame = ctk.CTkFrame(self, fg_color="transparent") if CTK_AVAILABLE else tk.Frame(self, bg="#0A0A12")
        self.add_label(self.gm_frame, "CONSOLE GAME MASTER", self.f_super, "#FF003C").pack(pady=(50, 20))
        gm_box = ctk.CTkFrame(self.gm_frame, fg_color="#12121A", corner_radius=20, border_width=2, border_color="#00E5FF") if CTK_AVAILABLE else tk.Frame(self.gm_frame, bg="#12121A", bd=3)
        gm_box.pack(expand=True, fill="both", padx=100, pady=20)
        self.lbl_gm_status = self.add_label(gm_box, "INIȚIALIZARE...", self.f_title, "#00E5FF")
        self.lbl_gm_status.pack(expand=True)
        self.add_button(self.gm_frame, "⛔ OPREȘTE MISIUNEA (STINGE TOT) ⛔", "transparent", "#FF003C", self.quit_to_menu, width=600, height=100, corner_radius=20, border_width=4, font=self.f_btn, text_color="#FF003C", hover_color="#440011").pack(pady=(0, 50))
    ...
    def add_button(self, parent, text, fg_col, border_col, command, text_color="white", **kwargs):
        if CTK_AVAILABLE: return ctk.CTkButton(parent, text=text, fg_color=fg_col, border_color=border_col, text_color=text_color, command=command, **kwargs)
        else: return tk.Button(parent, text=text, bg=fg_col if fg_col != "transparent" else "#222", fg=text_color, command=command, font=kwargs.get("font"))

    def hide_all(self):
        for f in [self.players_frame, self.diff_frame, self.details_frame, self.gm_frame, self.info_frame, self.test_frame]:
            f.place_forget()
    ...
    def start_rgb_test(self):
        self.hide_all()
        if CTK_AVAILABLE: self.test_frame.place(relx=0.5, rely=0.5, anchor="center")
        else: self.test_frame.pack(expand=True)
        self.test_running = True
        self.net = NetManager(self.target_ip, self.local_ip)
        threading.Thread(target=self._rgb_test_loop, daemon=True).start()

    def _rgb_test_loop(self):
        colors = [((255, 0, 0), "SE TRIMITE: ROȘU", "#FF0000"), ((0, 255, 0), "SE TRIMITE: VERDE", "#00FF00"), ((0, 0, 255), "SE TRIMITE: ALBASTRU", "#0000FF"), ((255, 255, 255), "SE TRIMITE: ALB", "#FFFFFF")]
        while self.test_running:
            for rgb, name, hexcode in colors:
                if not self.test_running: break
                if CTK_AVAILABLE: self.lbl_test_col.configure(text=name, text_color=hexcode)
                else: self.lbl_test_col.config(text=name, fg=hexcode)
                frame = {(c, l): rgb for c in range(1, 5) for l in range(11)}
                self.net.push(frame)
                time.sleep(3)

    def stop_rgb_test(self):
        self.test_running = False
        try:
            self.net.push({(c,l): (0,0,0) for c in range(1,5) for l in range(11)})
            time.sleep(0.1)
    ...
    def prep_game(self):
        try:
            ...
            s.sendto(pkt, ("255.255.255.255", 4626))
            s.close()
        except: pass

        self.hide_all()
        if CTK_AVAILABLE: self.gm_frame.place(relx=0.5, rely=0.5, anchor="center", relwidth=1, relheight=1)
        else: self.gm_frame.pack(expand=True, fill="both")
        
        self.scoreboard = ScoreboardWindow(self, self.selected_players, self.selected_difficulty, self.target_score)
        self.net = NetManager(self.target_ip, self.local_ip)
    ...
    def quit_to_menu(self):
        ...
        if hasattr(self, 'net'):
            try:
                self.net.push({(c,l): (0,0,0) for c in range(1,5) for l in range(11)})
                time.sleep(0.1); self.net.running = False
            except: pass
    ...
    def update_status(self, msg):
        if CTK_AVAILABLE: self.lbl_gm_status.configure(text=msg)
        else: self.lbl_gm_status.config(text=msg)
        if hasattr(self, 'scoreboard') and self.scoreboard.winfo_exists():
            self.scoreboard.update_status(msg)
    
    def update_score(self, s):
        if hasattr(self, 'scoreboard') and self.scoreboard.winfo_exists():
            self.scoreboard.update_score(s)
